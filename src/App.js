...
const App = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      const minScale = 0.5; // Minimum scaling factor
      const scale = Math.max(minScale, Math.min(window.innerWidth / 800, window.innerHeight / 800));
  ...
  return (
    <div className="app-container" style={{ transform: `scale(${scaleFactor})` }}>
      <div className="audio-player">
        <audio ref={audioRef} src="images/inst.mp3" />
        <button className="button-74" onClick={toggleAudio} style={{ fontSize: `${24 * scaleFactor}px`, padding: `${10 * scaleFactor}px ${20 * scaleFactor}px` }}>
          {isPlaying ? 'Pause' : 'Instruction'}
        </button>
      </div>

      <div className="timer" style={{ fontSize: `${20 * scaleFactor}px` }}>
        Timer: {formatTime(timer)}
      </div>

      <div className="button-container">
        <button className="button-74" onClick={prevLevel} disabled={currentLevel === 0} style={{ fontSize: `${24 * scaleFactor}px`, padding: `${10 * scaleFactor}px ${20 * scaleFactor}px` }}>
          Previous
        </button>
        <button className="button-74" onClick={nextLevel} disabled={currentLevel === levels.length - 1} style={{ fontSize: `${24 * scaleFactor}px`, padding: `${10 * scaleFactor}px ${20 * scaleFactor}px` }}>
          Next
        </button>
      </div>
